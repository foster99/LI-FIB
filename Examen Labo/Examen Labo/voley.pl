...
satVariable( hasDouble(T)  ):- team(T), !.                         % "team T has a double"
...
writeClauses(infinite):- !, numPlacedConstraints(N), writeClauses(N),!.
writeClauses(MaxUnplaced):-
    ...
    sixOrSevenHomes,           
    atMostOneDouble,
    atMostKUnplaced(MaxUnplaced),       % Constraint 5
    imposeHomes,                        % Constraint 6
    setDoubles,                         % Constraint 7
    atMost12TeamsHasADouble,
    true,!.
...
atMostKUnplaced(MaxUnplaced):-
    numPlacedConstraints(N), MinPlaced is N - MaxUnplaced,
    findall(match(T1,T2,R),(placed(T1-T2,R), team(T1), team(T2), round(R)), Lits),
    atLeast(MinPlaced,Lits),
    fail.
atMostKUnplaced(_).

imposeHomes:- home(T,L), round(R), member(R,L), writeClause([home(T,R)]), fail.
imposeHomes.

setDoubles:-
    team(T),
    findall(double(T,R), (round(R), R>1), Lits),
    expressOr(hasDouble(T), Lits),
    fail.
setDoubles.

atMost12TeamsHasADouble:- findall(hasDouble(T), team(T), Lits), atMost(12,Lits), fail.
atMost12TeamsHasADouble.
...
costOfThisSolution(M,Cost):-
    numPlacedConstraints(N),
    findall(_, (placed(T1-T2,R), member(match(T1,T2,R),M)), L),
    length(L, Placed),
    Cost is N - Placed,
    !.
...
main:-
    told, nl,nl,write('Looking for initial solution with arbitrary cost...'), nl,
...
treatResult(10,_):- %   shell('cat model',_),
    ...
    displaySol(M), 
    Cost1 is Cost-1,   nl,nl,nl,  write('Now looking for solution with cost '), write(Cost1), write('...'), nl,
